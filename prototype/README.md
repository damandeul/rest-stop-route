...
- 화면 1: 전국 장소 검색 데모, 출발지·목적지 교환, 3/5/10분, 그늘·벤치·냉방 실내·물·화장실 복수 선택, 유효성 오류
...
- 개인정보: 입력값은 브라우저 메모리에만 있고 서버 전송·localStorage 저장·분석 이벤트가 없다.
...
2. 전국 검색 데모의 주소·행정구역 표시
3. 3/5/10분 및 복수 휴식 조건, 시간 한도·조건 자료 하드 게이트
4. 휴식점 부족과 초과 구간·대안
5. 정보 부족과 시설 부재의 구분
6. 경로 계산 불가 시 임의 시간 미표시
7. 상세 출처·갱신일·신뢰 이유·운영정보 없음
8. 결과·상세 구간시간 일치와 대안 선택 44px 터치 영역
9. 실제 전송 없는 공유 미리보기
10. 입력·결과·상세 390px DOM 가로 오버플로 0
11. 390px 핵심 3화면 스크린샷 생성
12. 잘못 인코딩된 URL의 400 응답과 서버 생존
13. 인덱스 상태의 전량 60,672건·공식 출처
14. 좌표 주변 반경 검색과 냉방 `true/false/unknown` 3값 필터
15. 출처·원본 ID·수정시각·운영시간·경고 보존 및 `raw` 비노출
16. 잘못된 좌표·반경·필터 및 필수 좌표 누락·빈값의 HTTP 400
17. 목적지 주변 공식 실데이터의 브라우저 연결
18. API의 하드게이트 3상태·사유 보존과 `information_insufficient` 우선
19. `candidateOnly=true`의 `internal_route_candidate` 전용 필터
20. 서울 표본 상위 3건의 정보 부족·제한 사유·냉방 수량 확인 범위 표시
